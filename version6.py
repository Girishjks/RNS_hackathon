...
class VirusScannerApp:
    ...
    def display_result(self, result, virus_found, file_count, time_taken):
        ...
        for line in lines:
            if "Virus found" in line:
                ...
                infected_files.append(line.split("in file:")[1].strip())
            else:
        ...
        if virus_found:
            ...
            self.open_action_window(infected_files)
        else:
    ...
    def open_action_window(self, infected_files):
        action_window = tk.Toplevel(self.root)
        action_window.title("Action Required for Infected Files")
        action_window.geometry("800x400")
        
        ttk.Label(action_window, text="Select action for infected files:", font=("Helvetica", 16, "bold")).pack(pady=10)
        
        for index, file_path in enumerate(infected_files):
            file_frame = ttk.Frame(action_window)
            file_frame.pack(pady=5, padx=20, fill=tk.X)
            
            ttk.Label(file_frame, text=file_path, font=("Helvetica", 12)).grid(row=0, column=0, padx=10, pady=5)
            
            isolate_button = ttk.Button(file_frame, text="Isolate", command=lambda f=file_path: self.isolate_file(f, action_window))
            isolate_button.grid(row=0, column=1, padx=10, pady=5)
            
            delete_button = ttk.Button(file_frame, text="Delete", command=lambda f=file_path: self.delete_file(f, action_window))
            delete_button.grid(row=0, column=2, padx=10, pady=5)
            
            ignore_button = ttk.Button(file_frame, text="Ignore", command=lambda f=file_path: self.ignore_file(f, action_window))
            ignore_button.grid(row=0, column=3, padx=10, pady=5)
        
    def isolate_file(self, file_path, action_window):
        destination_dir = "C:\\Users\\Jayakumar\\OneDrive\\Desktop\\Isolation"
        os.makedirs(destination_dir, exist_ok=True)
        try:
            shutil.move(file_path, destination_dir)
            messagebox.showinfo("File Isolated", f"File {file_path} has been moved to quarantine.")
            action_window.destroy()
            self.update_text_area(f"File {file_path} isolated.\n")
        except Exception as e:
            messagebox.showerror("Error", f"Failed to isolate file {file_path}. Error: {str(e)}")

    def delete_file(self, file_path, action_window):
        try:
            os.remove(file_path)
            messagebox.showinfo("File Deleted", f"File {file_path} has been deleted.")
            action_window.destroy()
            self.update_text_area(f"File {file_path} deleted.\n")
        except Exception as e:
            messagebox.showerror("Error", f"Failed to delete file {file_path}. Error: {str(e)}")

    def ignore_file(self, file_path, action_window):
        action_window.destroy()
        self.update_text_area(f"File {file_path} ignored.\n")

    def update_text_area(self, message):
        self.text_area.insert(tk.END, message, "action")
